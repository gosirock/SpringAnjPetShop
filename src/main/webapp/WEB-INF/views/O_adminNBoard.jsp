...
<header>
	...
	<div class="page-title">
		...
		<h3>COMMUNITY</h3>
		<br>
		<br> <span class="selected">
		<a href="O_adminNotice">NOTICE
		</a></span> <a href="O_adminFAQ">FAQ</a> 
	...
	<div id="board-search">
		<div class="container">
			<div class="search-window">
				<form action="O_adminNotice" method="post">
			...
			<tbody>
				<c:forEach items="${noticeList}" var="dto">
					<tr>
						...
						<td><a href="O_adminNDetail?seq=${dto.seq}">${dto.n_title}</a></td>
						<td>${dto.adminid}</td>
						<td>${dto.writedate}</td>
					</tr>
				</c:forEach>
	...
	<div class="container" style="text-align: right;">
		<br> <span class="list-button"><a href="O_writeNoticeView">글쓰기</a></span>
		<br><br>
	</div>
    ...
    if (currentPage > pageSize) {
      document.write('<span><a href="O_adminNotice?page=' + 1 + '&query=' + query + '&content=${content}"><<</a></span>');
      document.write('<span><a href="O_adminNotice?page=' + ((Math.floor(currentPage/pageSize)+1)*pageSize + 1 - (pageSize * 2)) + '&query=' + query + '&content=${content}"><</a></span>');
    } else {
    ...
    if (totalPages != 1) { // 페이지 번호
	    for (let i = calcPage; i <= calcPage + pageSize - 1 && i <= totalPages; i++) {
	      if (i === currentPage) {
	    	document.write('<span class="current"><a href="O_adminNotice?page=' + i + '">' + i + '</a></span>');
	      } else {
	        document.write('<span><a href="O_adminNotice?page=' + i + '">' + i + '</a><span>');
	      }
	    }
	  }
    ...
    if ((currentPage != totalPages) && (totalPages != 1) && currentPage <= Math.floor(totalPages/pageSize)*pageSize && nextAndPrev <= totalPages) {
      document.write('<span><a href="O_adminNotice?page=' + nextAndPrev + '&query=' + query + '&content=${content}">></a><span>');
      document.write('<span><a href="O_adminNotice?page=' + totalPages + '&query=' + query + '&content=${content}">>></a><span>');
    }else{
      document.write('<span class="empty"><a>></a><span>');
      document.write('<span class="empty"><a>>></a><span>');
    }
