	...
	<div id="board-search">
		<div class="container">
			<div class="search-window">
				<form action="O_review" method="post">
...
<br><br><br>
		...
		<div class="review-grid">
			<c:forEach items="${reviewList}" var="dto">
				<div class="product-item">
					<a href="O_rDetail?seq=${dto.seq }"><img src="images/review/${dto.filename }.png" alt="이미지 준비 중"></a>
					...
					<h5>
						<a href="O_rDetail?seq=${dto.seq }" style="font-size: 17px;">${trimedTitle}</a>
					</h5>
    ...
    if (currentPage > pageSize) {
      document.write('<span><a href="O_review?page=' + 1 + '&query=' + query + '&content=${content}"><<</a></span>');
      document.write('<span><a href="O_review?page=' + ((Math.floor(currentPage/pageSize)+1)*pageSize + 1 - (pageSize * 2)) + '&query=' + query + '&content=${content}"><</a></span>');
    } else {
    ...
    if (totalPages != 1) { // 페이지 번호
	    for (let i = calcPage; i <= calcPage + pageSize - 1 && i <= totalPages; i++) {
	      if (i === currentPage) {
	    	document.write('<span class="current"><a href="O_review?page=' + i + '">' + i + '</a></span>');
	      } else {
	        document.write('<span><a href="O_review?page=' + i + '">' + i + '</a><span>');
	      }
	    }
	  }
